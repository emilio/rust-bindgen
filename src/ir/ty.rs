...
impl Type {
    ...
    pub fn signature_contains_named_type(&self,
        ...
        match self.kind {
            TypeKind::Named(ref this_name)
                => this_name == name,
            TypeKind::Array(ref t, _) |
            TypeKind::Pointer(ref t)
                => type_resolver.resolve_type(*t)
...
impl Type {
        ...
        let kind = match ty.kind() {
            ...
            CXType_Invalid => {
                ...
                if ty.ret_type().kind() != CXType_Invalid {
                    let signature =
                        FunctionSig::from_ty(ty, &cursor, ctx)
                ...
                // this is a Comp(..)
                    ...
                    match location.kind() {
                        ...
                        CXCursor_TypeRef => {
                            ...
                            return Type::from_clang_ty(potential_id,
                                                       &referenced.cur_type(),
                                                       Some(referenced),
                                                       parent_id,
                                                       ctx);
    ...
    fn template_arg_count(&self) -> usize {
        if let TypeKind::Comp(ref ci) = self.kind {
            ci.template_args().len()
        } else {
            0
        }
    }
